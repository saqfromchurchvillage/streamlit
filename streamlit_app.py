...
if not st.session_state.reviews.empty:
    beer_names = sorted(set(st.session_state.reviews['Oluen nimi'].tolist()))
else:
    beer_names = []
...
beer_name_options = beer_names + ["Lisää uusi olut"]

# Oluen nimen valinta pudotusvalikosta
selected_option = st.sidebar.selectbox("Valitse oluen nimi", beer_name_options)

if selected_option == "Lisää uusi olut":
    # Jos käyttäjä valitsee "Lisää uusi olut", näytetään tekstikenttä uuden oluen nimen syöttämiseen
    new_beer_name = st.sidebar.text_input("Syötä uuden oluen nimi")
    beer_name = new_beer_name.strip()
    if beer_name:
        st.sidebar.write(f"**Valittu olut:** {beer_name}")
    else:
        st.sidebar.warning("Syötä uuden oluen nimi.")
else:
    beer_name = selected_option
    st.sidebar.write(f"**Valittu olut:** {beer_name}")
...
if st.sidebar.button("Submit"):
    if beer_name and arvostelijan_nimi:
        ...
        if beer_name not in beer_names:
...
top_beers = st.session_state.reviews.groupby("Oluen nimi").agg(
